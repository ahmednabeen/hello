...
def home_view(request):
    ...
    is_search = bool(doctor_name_query or hospital_name_query or department_query or location_query)
    ...
    if is_search:
        ...
        if department_query:
            doctors_qs = doctors_qs.filter(specialties__department__slug=department_query).distinct()
        ...
        if location_query:
            if 'name' in request.GET or 'department' in request.GET:
    ...
    if cached and not is_search:
        featured_doctors, featured_hospitals, featured_specialties, featured_departments, featured_departments_all, doctors_count, hospitals_count, locations_count, locs, departments = cached
    else:
        ...
        if is_search:
            ...
            cache.set(cache_key, (featured_doctors, featured_hospitals, featured_specialties, featured_departments, featured_departments_all,
                      doctors_count, hospitals_count, locations_count, locs, departments), CACHE_TTL)
    ...
    context = {
        ...
        'department_value': department_query,
...
def top_doctors_view(request, department=None, location=None):
    department_slug = request.GET.get('department', '') or request.GET.get('specialty', '') or department
    location_slug = request.GET.get('location', '') or location
    ...
    paginator = Paginator(doctors, count)
...
def top_hospitals_view(request, location=None):
    ...
    query_parsed_into_filter = False
    ...
    if query and not query_parsed_into_filter:
